...
export const FleetApp = {
  ...
  mount(el) {
    ...
    box.addEventListener('change', e => {
      ...
      if (!cb) return;
      setRepeat(Number(cb.dataset.nr), cb.checked);
      onTick();
    });
  },
  ...
  update(el) {
    ...
    el.querySelector('#flNote').textContent =
      `${S.trucks.length} LKW · ${S.trucks.filter(t => t.phase === 'idle' && !t.shopMin).length} im Depot`;

    const sig = S.trucks.map(t =>
      [t.nr, t.driver.level, t.driver.points, Object.values(t.driver.skills).join(''),
       t.phase, t.repeat ? 1 : 0, t.shopMin > 0 ? 1 : 0].join(':')).join('|');
    ...
    for (const truck of S.trucks) {
      ...
      if (truck.shopMin > 0) {
        ...
        status.innerHTML = `→ <strong>${esc(truck.order.firm.name.slice(0, 20))}</strong>`;
  ...
  <div class="truck-row">
    ...
    <div class="flex-row" style="justify-content:space-between;font-size:10px;">
      <label class="flex-row" style="gap:3px;">
        <input type="checkbox" data-nr="${truck.nr}" ${truck.repeat ? 'checked' : ''}>
        Dauerauftrag
      </label>
